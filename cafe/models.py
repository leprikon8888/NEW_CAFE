...
# все одинаковіе параметрі віношу, чтобі не дублировать код
...
class DishCategory(Same_settings):
    category = models.ForeignKey(Menu, on_delete=models.PROTECT, related_name='dishcategories')
...
class Event(Same_settings):
    category = models.ForeignKey(DishCategory, on_delete=models.PROTECT, related_name='events')
    desc = models.TextField(max_length=1500, blank=True)
...
class Shef(Same_settings):
    ...
    category = models.ForeignKey(DishCategory, on_delete=models.PROTECT, related_name='shefs')
